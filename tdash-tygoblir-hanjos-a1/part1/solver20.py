...
#
# Code by: Josep Han (hanjos), Tarini Dash (tdash), Tyler Goblirsch (tygoblir)
#
...
def state_heuristic(state):
    result = []
    rowsum,colsum =(0,0)
    misplaced_row, misplaced_col = (0,0)
    ...
    # if misplaced_row ==0 and colsum %5 != 0 :
    #     raise(Exception("Invalid configuration. Row organized, but column not a 5 multiple. Row sum = ", misplaced_row,'Col sum= ', misplaced_col, state))
    # if misplaced_col == 0 and misplaced_row == 3:
    #     raise(Exception("Invalid configuration. Column organized, but row not a 4 multiple. Row sum = ", misplaced_row,'Col sum= ', misplaced_col, state))
...
def solve2(initial_board):
    ...
    while not fringe.empty():
        ...
        # It is not necessary for calculating heuristics, but as benchmark for time constraints
        # icounter += 1
        # print(icounter)
        # initheur = got[0]
        if is_goal(state):
        ...
        for (succ,move) in successors(state):
            ...
            fringe.put((len(route_so_far)+ heuristic_of_succ, tt))
start_state = []
